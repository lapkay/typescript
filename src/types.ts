...
export type AutorWoEmail = Omit<Author, 'Email'>;
...
export type RequiredProp<T extends object> = {
   [prop in keyof T]: {} extends Pick<T, prop> ? never : prop }[keyof T];

export type OptionalProp<T extends object> = {
   [prop in keyof T]: {} extends Pick<T, prop> ? prop : never }[keyof T];
type BookRequiredProps = RequiredProp<Book>;
type BookOptionalProps = OptionalProp<Book>;
